...
create table if not exists public.reservations_hackathon (
  ...
  -- No FK to rate_plan_lookup: the site's detail pages publish raw
  -- channel-specific rate codes (e.g. OCHEARLY, EXPBARH) that are absent from
  -- the 8-row published lookup, with no alias mapping anywhere on the site.
  -- We load the source value faithfully and left-join the lookup where a plan
  -- family is needed. See ARCHITECTURE.md §1.
